...
const ProjectFilters: React.FC<ProjectFiltersProps> = ({
  ...
  return (
    <div className="space-y-4 mb-6 bg-gray-50 p-4 rounded-lg">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-2">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          >
            ...
            {isMobileView ? "Filters" : "Advanced Filters"}
          </Button>
      ...
      {(filters.status.length > 0 ||
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {(filters.budgetRange.min || filters.budgetRange.max) && (
            <Badge variant="outline" className="flex items-center gap-1">
              Budget: ${filters.budgetRange.min || 0} - $
              {filters.budgetRange.max || "∞"}
              <X
                className="h-3 w-3 cursor-pointer"
                onClick={() =>
      ...
      {isAdvancedOpen && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4 border rounded-lg bg-white">
          {/* Status filter */}
