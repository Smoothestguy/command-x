...
export const registerUser = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  console.log(`[registerUser] Checking existence for username: ${username}, email: ${email}`);
  ...
  const newUserResult = await query(
    ...
    [username, passwordHash, email, firstName, lastName, role || 'FieldStaff', 'Active'] 
  );
...
export const loginUser = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!username || !password) {
    console.log('[loginUser] Validation failed: Missing username or password.');
    return res.status(400).json({ message: 'Username and password are required.' });
  }

  console.log(`[loginUser] Finding user by username: ${username}`);
  const userResult = await query('SELECT user_id, username, password_hash, email, role, status, first_name, last_name, created_at FROM users WHERE username = $1', [username]);
  ...
  console.log(`[loginUser] User found: ${user.username}. Comparing password...`);
  ...
  console.log(`[loginUser] Password matched for user: ${username}. Generating JWT...`);
